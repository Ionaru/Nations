...
public class CmdNations implements CommandExecutor {
    ...
    private boolean nationList(CommandSender cs){
        cs.sendMessage(parent.colorize("&f---- &bActive nations &f----"));
        for(NationType n: NationType.values()){
            cs.sendMessage(parent.colorize("&b" + n.getTitle() + "&f | &aUse &6/nation info " + n.getTitle() + "&a for more information."));
        }
        return true;
    }
    ...
    private boolean nationLeave(CommandSender cs){
        ...
        parent.removeFromNation((Player)cs);
        return true;
    }
    ...
    private boolean nationInfo(CommandSender cs, String[] args){
        ...
        if(!NationType.contains(nation)){
        	cs.sendMessage(parent.colorize("&cThat nation does not exist, use &6/nation list &cto see the active nations!"));
            return true;
        }
        NationType type = NationType.valueOf(nation);
        cs.sendMessage(parent.colorize("&f---- &6Nation info: &b" + type.getTitle() + " &f----"));
        cs.sendMessage(parent.colorize("&6Lore: &a" + type.getLore()));
        cs.sendMessage(parent.colorize("----"));
        cs.sendMessage(parent.colorize("&6Traits: &a" + type.getTraits()));
        cs.sendMessage(parent.colorize("----"));
        return true;
    }
    ...
    private boolean nationJoin(CommandSender cs, String[] args){
        ...
        parent.addToNation(NationType.valueOf(nation), (Player) cs);
        return true;
    }
